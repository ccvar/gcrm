...
2) 整理成人看的 Markdown 表格：公司名｜对接人/部门｜公开联系方式｜来源链接｜为什么是潜客(一句话)。\n\
3) 在表格之后，另起一段附上机器可读的线索数据（供 Pilot「一键入池」用，用户能看到但不用手动处理）。\
格式严格如下，只放你确实找到的公开线索，找不到的字段留空字符串：\n\
<<<LEADS>>>\n\
[{\"company\":\"公司名\",\"contact\":\"对接人/部门\",\"phone\":\"电话\",\"email\":\"邮箱\",\"wechat\":\"微信\",\"source\":\"来源(如 行业名录)\",\"source_url\":\"来源链接\",\"reason\":\"为什么是潜客\"}]\n\
<<<END>>>\n\
4) 结尾提示用户：确认后点「入池」把线索收进客户池；池里可再逐条「推入 GCRM」建成客户（需先连接）。\
你自己不要调用任何写入接口，入池与推送都由用户在界面上操作——这是关键动作，必须人确认。\n",
            );
        }
        _ => {}
    }
